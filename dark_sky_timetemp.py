...
class DarkSkyTimeTemp:
    ...
    def update_and_print(self):
        ...
        wind_speed = int(data.currently.windSpeed)
        wind_dir = data.currently.windBearing

        log.debug("Temperature [%s] humidity [%s] wind speed [%s] wind direction [%s] wind units [%s]", temperature, humidity, wind_speed, wind_direction)
        ...
        def croplist(widths, x, y, height):
            ...
            for i in range(len(widths)):
                list.append(symbols.crop([x, y + i * height, x + widths[i], y + (i + 1) * height]))
            return list
        ...
        if DayWidth[t.tm_wday] > w:
            w = DayWidth[t.tm_wday]
        ...
        if wind_speed > 0:
            for winDirNum in range(len(DirAngle) - 1):
                if wind_dir < DirAngle[winDirNum]:
                    break
        winDirNum += 1
        ...
        self._printer.printImage(img, True)
