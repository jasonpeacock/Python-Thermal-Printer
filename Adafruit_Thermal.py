...
class Adafruit_Thermal(Serial):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.writeToStdout is False:
            ...
            self.writeBytes(
                27, 55, 11, heatTime, 40  # Esc  # 7 (print settings)  # Heat dots  # Lib default
            )  # Heat interval
            ...
            self.writeBytes(18, 35, (printBreakTime << 5) | printDensity)  # DC2  # Print density
    ...
    def writeBytes(self, *args):
        ...
        if self.writeToStdout:
            for arg in args:
                sys.stdout.write(chr(arg).encode('latin-1'))
        else:
            self.timeoutWait()
    ...
    def write(self, *data):
        ...
        for i in range(len(data)):
            c = data[i]
            if self.writeToStdout:
                sys.stdout.write(c.encode('latin-1'))
                continue
            if c != 0x13:
                self.timeoutWait()
    ...
    def printBarcode(self, text, type):
        ...
        self.writeBytes(
            29, 72, 2, 29, 119, 3, 29, 107, n  # Print label below barcode  # Barcode width
        )  # Barcode type
        self.timeoutWait()
        ...
        if self.firmwareVersion >= 264:
            ...
            if n > 255:
                n = 255
            if self.writeToStdout:
                sys.stdout.write(chr(n).encode('latin-1'))
                for i in range(n):
            ...
            if self.writeToStdout:
                sys.stdout.write(text.encode('latin-1'))
            else:
    ...
    def printImage(self, image, LaaT=False):
        ...
        Requires Python Imaging Library. This is specific to the Python port and not present in
        the Arduino library. Image will be cropped to 384 pixels width if necessary, and converted
        to 1-bit w/diffusion dithering. For any other behavior (scale, B&W threshold, etc.), use
        the Imaging Library to perform such operations before passing the result to this function.
        """
    ...
    def listen(self):
        """Empty method, included for compatibility with existing code ported from Arduino."""
        pass

    def hasPaper(self):
        """Check the status of the paper using the printers self reporting ability. Doesn't match the datasheet...
    ...
    def print(self, *args, **kwargs):
        """Overloading print() in Python pre-3.0 is dirty pool, but these are here to provide more direct compatibility
