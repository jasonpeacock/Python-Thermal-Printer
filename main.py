...
LED_PIN = 18
BUTTON_PIN = 23
HOLD_TIME_SECONDS = 2  # Duration for button hold (shutdown)
TAP_TIME_SECONDS = 0.01  # Debounce time for button taps
...
def interval():
    """Called at periodic intervals (30 seconds by default). Invokes twitter script."""
    GPIO.output(LED_PIN, GPIO.HIGH)
    p = subprocess.Popen(["./twitter.py", str(lastId)], stdout=subprocess.PIPE)
    GPIO.output(LED_PIN, GPIO.LOW)
...
try:
    ...
    s.connect(("8.8.8.8", 0))
    printer.print("My IP address is {}".format(s.getsockname()[0]))
    printer.feed(3)
except:
    printer.boldOn()
...
while True:
    ...
    if buttonState != prevButtonState:
        ...
        if (t - prevTime) >= HOLD_TIME_SECONDS:  # Button held more than 'HOLD_TIME_SECONDS'?
            # Yes it has.  Is the hold action as-yet untriggered?
            if holdEnable == True:  # Yep!
                hold()  # Perform hold action (usu. shutdown)
                holdEnable = False  # 1 shot...don't repeat hold action
                tapEnable = False  # Don't do tap action on release
        elif (t - prevTime) >= TAP_TIME_SECONDS:  # Not HOLD_TIME_SECONDS.  TAP_TIME_SECONDS elapsed?
    ...
    if t > nextInterval:
        ...
        if result is not None:
            lastId = result.decode("utf-8").rstrip("\r\n")
